...
# --- FUNCIONES DE CARGA (AQUÍ ESTABA EL ERROR) ---

def cargar_historial_completo(sheet):
    """Carga segura del historial de cierres."""
    try:
...
def show(sheet):
    ...
    with tab_ahorro:
        ...
        if not df_c.empty:
            ...
            if not df_c[df_c["Estado_Ahorro"] == "PENDIENTE"].empty:
                st.markdown("### 🟠 Pagar Pendientes")
                opciones = df_c[df_c["Estado_Ahorro"] == "PENDIENTE"].apply(lambda x: f"{x['Fecha']} - {formato_moneda_co(x['Profit_Retenido'])}", axis=1).tolist()
                pago = st.selectbox("Pagar:", opciones)
