...
def show(sheet):
    ...
    with tab1:
        ...
        if datos_cierre is not None:
            ...
            if df_ventas is not None and not df_ventas.empty:
                ...
                with st.expander("🛠️ Corregir Medios de Pago", expanded=False):
                    ...
                    if st.button("💾 Guardar Correcciones"):
                        ...
                        if actualizar_metodos_pago(ws_v, df_edit): st.success("Listo."); st.rerun()
            ...
            st.markdown("---")
            # PROFIT (SOLO CÁLCULO)
            st.subheader("💰 Cálculo Profit First")
            c_p1, c_p2 = st.columns([1, 2])
            pct_prof = c_p1.number_input("% Ahorro", value=5, min_value=1)
            monto_p = v_total * (pct_prof/100)
            c_p2.info(f"👉 Se enviará una deuda de **{formato_moneda_co(monto_p)}** al módulo 'Banco Profit'.")
            ...
            if st.button("🔒 CERRAR CAJA", type="primary", use_container_width=True):
                datos_dict = {
                    ...
                    "Notas": not_cierre, 
                    "Profit_Retenido": monto_p, # Se guarda el cálculo
                    "Estado_Ahorro": "PENDIENTE", # Siempre nace pendiente
    ...
    with tab2:
        ...
        if not df_full.empty:
            ...
            st.dataframe(df_ver[["Fecha", "Numero_Cierre_Loyverse", "Saldo_Real_Contado", "Diferencia", "Notas"]].sort_values("Fecha", ascending=False), use_container_width=True, hide_index=True)
        else: st.info("Sin cierres.")
