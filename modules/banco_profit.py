...
# Estructura exacta de tus hojas
...
def cargar_datos_banco(sheet):
    try:
        ...
        # 2. Cargar Abonos con validación de columnas
        ws_a = sheet.worksheet(HOJA_ABONOS)
        df_a = leer_datos_seguro(ws_a)
        if df_a.empty: df_a = pd.DataFrame(columns=HEADERS_ABONOS)
        
        # 3. Cargar Retiros con validación de columnas
        ws_r = sheet.worksheet(HOJA_RETIROS)
        df_r = leer_datos_seguro(ws_r)
        if df_r.empty: df_r = pd.DataFrame(columns=HEADERS_RETIROS)
        ...
        # Asegurar que Abonos y Retiros tengan sus columnas numéricas limpias
        if "Monto_Abonado" in df_a.columns:
        ...
        return pd.DataFrame(), pd.DataFrame(columns=HEADERS_ABONOS), pd.DataFrame(columns=HEADERS_RETIROS), None, None, None

def registrar_abono(ws_a, fecha_cierre, monto, resp):
    try:
        fila = [generar_id(), datetime.now(ZONA_HORARIA).strftime("%Y-%m-%d"), 
                datetime.now(ZONA_HORARIA).strftime("%H:%M"), str(fecha_cierre), monto, resp]
...
def registrar_retiro(ws_r, monto, motivo, resp):
    try:
        fila = [generar_id(), datetime.now(ZONA_HORARIA).strftime("%Y-%m-%d"), 
                datetime.now(ZONA_HORARIA).strftime("%H:%M"), monto, motivo, resp]
...
def show(sheet):
    ...
    # --- CÁLCULOS ---
    total_ahorrado = df_a["Monto_Abonado"].sum() if "Monto_Abonado" in df_a.columns else 0
    total_retirado = df_r["Monto_Retirado"].sum() if "Monto_Retirado" in df_r.columns else 0
    ...
    with t1:
        ...
        if not pendientes.empty:
            ...
            monto_real = col_a.number_input("Cantidad a ahorrar", value=float(monto_sug), step=1000.0)
            ...
            if st.button("✅ GUARDAR ABONO"):
                if monto_real > 0:
                    if registrar_abono(ws_a, fecha_sel, monto_real, responsable):
                        if marcar_completado: marcar_cierre_como_ahorrado(ws_c, fecha_sel)
    ...
    with t2:
        ...
        if st.button("🚨 EJECUTAR RETIRO"):
            if 0 < monto_ret <= saldo_disponible:
                ...
                st.error("Saldo insuficiente o datos incompletos.")

    with t3:
        st.subheader("Historial de Movimientos")
        
        # Historial de entradas (Abonos)
        entradas = pd.DataFrame()
        if not df_a.empty:
            columnas_a = [c for c in ["Fecha", "Monto_Abonado", "Fecha_Cierre_O", "Responsable"] if c in df_a.columns]
            entradas = df_a[columnas_a].copy()
            entradas.columns = ["Fecha", "Monto", "Referencia", "Responsable"]
            entradas["Tipo"] = "ENTRADA"

        # Historial de salidas (Retiros)
        salidas = pd.DataFrame()
        if not df_r.empty:
            columnas_r = [c for c in ["Fecha", "Monto_Retirado", "Motivo", "Responsable"] if c in df_r.columns]
            salidas = df_r[columnas_r].copy()
        ...
        if not entradas.empty or not salidas.empty:
            ...
            def color_tipo(val):
                return f'color: {"green" if val == "ENTRADA" else "red"}; font-weight: bold'
